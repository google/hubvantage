 ...
 */
const REACH_BY_FREQUENCY_AND_MONTH_QUERY_TEMPLATE = `
with
  impressions_by_custom_grouping AS (
    ...
    LEFT JOIN
      (SELECT advertiser, advertiser_id FROM adh.cm_dt_advertiser)
        AS advertiser_mt
      ON imp.event.advertiser_id = advertiser_mt.advertiser_id
    LEFT JOIN
      (SELECT campaign, campaign_id FROM adh.cm_dt_campaign) AS campaign_mt
      ON imp.event.campaign_id = campaign_mt.campaign_id
    LEFT JOIN
      (SELECT placement, placement_id FROM adh.cm_dt_placement) AS placement_mt
      ON imp.event.placement_id = placement_mt.placement_id
    LEFT JOIN
      (SELECT site, site_id FROM adh.cm_dt_site) AS site_mt
      ON imp.event.site_id = site_mt.site_id
    LEFT JOIN
      (SELECT browser_platform, browser_platform_id FROM adh.cm_dt_browser)
        AS browser_mt
      ON
        cast(imp.event.browser_platform_id as string)
        = cast(browser_mt.browser_platform_id as string)
    LEFT JOIN
      (SELECT dma_region, dma_region_id FROM adh.cm_dt_designated_market_area)
        AS dma_mt
  ...
  clicks_by_custom_grouping AS (
    ...
    LEFT JOIN
      (SELECT browser_platform, browser_platform_id FROM adh.cm_dt_browser)
        AS browser_mt
      ON
        cast(clicks.event.browser_platform_id as string)
        = cast(browser_mt.browser_platform_id as string)
    LEFT JOIN
      (SELECT dma_region, dma_region_id FROM adh.cm_dt_designated_market_area)
        AS dma_mt
  ...
  table_joined AS (
    SELECT
      User_ID,
      Concat(
        Cast(EXTRACT(YEAR FROM TIMESTAMP_MICROS(Interaction_Time)) as string),
        "-",
        CAST(EXTRACT(MONTH FROM TIMESTAMP_MICROS(Interaction_Time)) as string),
        "-01"
      ) As Month,
...
FROM table_joined GROUP BY 1, 2 , __DYNAMIC_REPORTING__SET_NAME_TAG__
`;
